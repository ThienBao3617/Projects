...
static esp_err_t stream_handler(httpd_req_t *req){
  ...
  /* Loop to show streaming video from ESP32 Cam and read QR Code. */
  while(true){
    ws_run = true;
...
static esp_err_t cmd_handler(httpd_req_t *req){
  ...
  // S = Slider
  // 10 = slider value
  // set the slider value range from 0 to 20.
...
void QRCodeReader( void * pvParameters ){
  ...
  while(!ws_run){
      ...
      quirc_resize(q, firebase->width, firebase->height);
      image = quirc_begin(q, NULL, NULL);
      memcpy(image, firebase->buf, firebase->len);
      quirc_end(q);
